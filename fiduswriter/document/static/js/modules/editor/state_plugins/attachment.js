...
export let attachmentPlugin = function(options){
//prosemirror
    return new Plugin ({
    key,
    state: {
        init(config, state) {
          // This array is used to store the links
          let link_array = []
          // This array is used to store the elements itself. TO be used in future for better error handling
          let element_array = []
          let is_template_item_present = false
          console.log("#####################################3")
          let dragSrcEl = null;


          if(!options.editor.docInfo.template){
              console.log("#####################################3")
              let content = options.editor.docInfo.template.content
      
              // This is used as way to check if the template is phase2 based on element id
              // Class name is hard coded and must exist in the template
              let at_pos = document.querySelector(".article-part-button");
              if(at_pos !== null){
                  let docId = options.editor.docInfo.id
                  let manage_att = document.createElement('h4')
                  manage_att.innerHTML="Manage Attachment"
                  at_pos.appendChild(manage_att)
                  
                  //let manage_attachment = document.querySelector(".article-Letters_of_intent_opt_buttons h4:nth-child(2)");

                  // Add condition for checking access rights
                  if(options.editor.docInfo.access_rights == 'write' || options.editor.docInfo.access_rights == 'write-tracked'){
                      manage_attachment.onclick = ()=>{
                          show_manage_attachment_dialog(docId = docId)
                      }
                  }
                  else{
                      manage_attachment.onclick = ()=>{
                        addAlert("info","Only collaborators with write access rights can upload/manage documents.")  
                    }
                  }
      
                  let upload_att = document.createElement('h4')
                  upload_att.innerHTML="Upload Attachment"
                  console.log("access rights :- ", at_pos ,options.editor.docInfo.access_rights )
                  // Add condition for checking access rights
                  if(options.editor.docInfo.access_rights == 'write' || options.editor.docInfo.access_rights == 'write-tracked'){
                      upload_att.onclick = function () {
                        show_dialog_attach(docId = docId)
                      }
                  }
                  else{
                      upload_att.onclick = function () {
                ...
                //}
                // if(content.length > 0)
                //   clearInterval(check_template)
              }
              function handleDragStart(e) {
                // Target (this) element is the source node.
                dragSrcEl = options;
                e.dataTransfer.effectAllowed = 'move';
                e.dataTransfer.setData('text/html', options.outerHTML);
                options.classList.add('dragElem');
              }
              ...
              function handleDragOver(e) {
                ...
                options.classList.add('over');
                if(options.classList.contains('delete-area')){
                  options.classList.add('delete-area-drop')
                }
              ...
              function handleDragLeave(e) {
                ...
                // console.log(this)
                if(options.classList.contains('delete-area')){
                  options.classList.remove('delete-area-drop')
                }
                options.classList.remove('over');  // this / e.target is previous target element.
              }
              ...
              function handleDrop(e) {
                ...
                if (dragSrcEl != this) {
                  if(options.classList.contains('delete-area')){
                    ...
                    options.previousElementSibling.removeChild(dragSrcEl)
                    options.classList.remove('delete-area-drop')
                    // console.log("hola",dragSrcEl)
                    return;
                  }
                  // Set the source column's HTML to the HTML of the column we dropped on.
                  //alert(options.outerHTML);
                  //dragSrcEl.innerHTML = options.innerHTML;
                  //options.innerHTML = e.dataTransfer.getData('text/html');
                  options.parentNode.removeChild(dragSrcEl);
                  var dropHTML = e.dataTransfer.getData('text/html');
                  options.insertAdjacentHTML('beforebegin',dropHTML);
                  var dropElem = options.previousSibling;
                  addDnDHandlers(dropElem);    
                }
                options.classList.remove('over');
                return false;
              }
              ...
              function handleDragEnd(e) {
                ...
                options.classList.remove('over');
                options.classList.remove('dragElem');
...
// This array is used to store the links          console.log(options)
let link_array = []
// This array is used to store the elements itself. TO be used in future for better error handling
...
export class Attachment {
  ...
  init() {
    ...
    whenReady().then(()=>{
      ...
      let check_template = setInterval(()=>{

        if(this.editor.docInfo.template){
          console.log("#####################################3")
          // let content = this.editor.docInfo.template.content

          //       // This is used as way to check if the template is phase2 based on element id
          //       // Class name is hard coded and must exist in the template
          //       let at_pos = document.querySelector(".article-part-button");
          //       if(at_pos !== null){
          //         let docId = this.editor.docInfo.id

          //         let manage_attachment = document.querySelector(".article-Letters_of_intent_opt_buttons h4:nth-child(2)");
          //         // Add condition for checking access rights
          //         if(manage_attachment !== null && (this.editor.docInfo.access_rights == 'write' || this.editor.docInfo.access_rights == 'write-tracked')){
          //           manage_attachment.onclick = ()=>{
          //             show_manage_attachment_dialog(docId = docId)
          //           }
          //         }
          //         else if (manage_attachment !== null){
          //           manage_attachment.onclick = ()=>{
          //             addAlert("info","Only collaborators with write access rights can upload/manage documents.")  
          //           }
          //         }
          ...
          //         console.log("AT POS :- ", at_pos ,this.editor.docInfo.access_rights )
          //         // Add condition for checking access rights
          //         if(this.editor.docInfo.access_rights == 'write' || this.editor.docInfo.access_rights == 'write-tracked'){
          //           at_pos.onclick = function () {
          //             show_dialog_attach(docId = docId)
          //           }
          //         }
          //         else{
